...
void main(void)
{
 ...
 // get edge vectors of the pixel triangle
   ...
   vec3 base = texture2D(diffuseTexture, TexCoord).xyz;
   vec3 bump = texture2D(normalTexture, TexCoord).xyz;
...
/*
	...
	vec3 lightVec = vLightDir - vViewDir;  
	float cosinus = dot(myNormal, viewDir); 
*/
	finalColor = (ambient + diffuse + specular) * LightIntensity;
